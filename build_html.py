...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .toolbar label {
    ...
    min-width: 0; /* let grid tracks shrink below option-text width so controls stay inside the card */
  }
  .toolbar label input, .toolbar label select { width: 100%; min-width: 0; }
...
function flagBadges(c) {
  ...
  // Decision markers are decider-facing only; the public view shows effective
  // values without exposing that (or how) a district decision was made.
  if (MODE === "decisions" && eff.decision) {
...
function detailRow(c, eff) {
  ...
  const scopeLabel = decided && decided.applies_to
    ...
    : "";
  const decidedNote = (MODE === "decisions" && decided)
